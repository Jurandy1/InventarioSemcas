...
export function CoordenadoresTab({ unidades, showT, isMob }) {
  ...
  const handleGerarConvite = async () => {
    ...
    try {
      const { convite, link } = await gerarLinkConviteCoordinador(novoConvite.unidadeId, "", novoConvite.matricula);
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
        ...
        <button onClick={() => { setModal("novoconvite"); setForm({ motivo: "", observacoes: "" }); }} style={bp}>
          + Gerar Convite
        </button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMob ? "1fr" : "repeat(auto-fill, minmax(360px,1fr))", gap: 10 }}>
          ...
          (coordAprovadas.length === 0 ? (
            ...
            coordAprovadas.map((coord) => (
              <CoordCard
                ...
                action={{
                  buttons: [
                    { label: "🗑 Desativar", action: "desativar", color: "#dc2626" },
                  ],
                  onAction: (action) => {
                    setSelectedCoord(coord);
                    setForm({ motivo: "", observacoes: "" });
        ...
        {tab === "rejeitadas" &&
          (coordRejeitadas.length === 0 ? (
            ...
            coordRejeitadas.map((coord) => (
              <CoordCard key={coord.uid} coord={coord} action={null} />
            ))
          ))}
      </div>
      ...
      {modal === "aprovar" && selectedCoord && (
        <Overlay onClose={() => setModal(null)}>
          ...
          <p style={{ margin: "0 0 12px", fontSize: 13, color: "#64748b" }}>Tem certeza que quer aprovar <strong>{selectedCoord.nome}</strong>?</p>

          <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#374151", marginBottom: 6 }}>Observações (opcional)</label>
          <TArea initial={form.observacoes} onVal={(v) => setForm((prev) => ({ ...prev, observacoes: v }))} rows={3} placeholder="Ex: Validado com RH" style={{ ...inp, resize: "none" }} />
      ...
      {modal === "rejeitar" && selectedCoord && (
        <Overlay onClose={() => setModal(null)}>
          ...
          <p style={{ margin: "0 0 12px", fontSize: 13, color: "#64748b" }}>Você está rejeitando <strong>{selectedCoord.nome}</strong></p>

          <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#374151", marginBottom: 6 }}>Motivo da rejeição *</label>
          <TArea initial={form.motivo} onVal={(v) => setForm((prev) => ({ ...prev, motivo: v }))} rows={3} placeholder="Ex: Matrícula inválida, dados incompletos..." style={{ ...inp, resize: "none" }} />
      ...
      {modal === "desativar" && selectedCoord && (
        <Overlay onClose={() => setModal(null)}>
          ...
          <p style={{ margin: "0 0 12px", fontSize: 13, color: "#64748b" }}>Você está desativando <strong>{selectedCoord.nome}</strong>. Ela não conseguirá mais acessar o sistema.</p>

          <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#374151", marginBottom: 6 }}>Motivo (opcional)</label>
          <TArea initial={form.motivo} onVal={(v) => setForm((prev) => ({ ...prev, motivo: v }))} rows={3} placeholder="Ex: Desligamento, mudança de cargo..." style={{ ...inp, resize: "none" }} />
      ...
      {modal === "novoconvite" && (
        <Overlay onClose={() => setModal(null)}>
          ...
          <select
            value={novoConvite.unidadeId}
            onChange={(e) => setNovoConvite((prev) => ({ ...prev, unidadeId: e.target.value }))}
            style={inp}
          >
          ...
          <TInput
            initial={novoConvite.matricula}
            onVal={(v) => setNovoConvite((prev) => ({ ...prev, matricula: v }))}
