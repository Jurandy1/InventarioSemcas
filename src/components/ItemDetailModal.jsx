...
export function ItemDetailModal({
  ...
  const tipoRegistro = getTipoRegistroItem(item, foundEntry);
  ...
  return (
    <>
      ...
      <p style={{ margin: "0 0 4px", fontSize: 12, color: "#64748b" }}>
        Nº {getItemCode(item)} · {tipoRegistro} · {item.data} · R$ {(item.valor || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
      </p>
      {idInterno ? (
